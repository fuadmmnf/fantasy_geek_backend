...
class FixtureProgressTracker
{
    ...
    {
        $this->scorecards = $this->fixture->scorecards;
        $this->calculateAndStorePoints($fixtureDTO);

        foreach ($this->fixture->contests as $contest) {
            $this->usercontests = $contest->usercontests;
            $this->updateUserContestProgress();
            $contestStandings = array();
            $this->usercontests = $this->usercontests->sortByDesc('score');
            $j = 0;
    ...
    private function calculateAndStorePoints(FixtureDetailDTO $fixtureDetailDTO)
    {
        $this->scorecards = $this->scorecards->transform(function ($scorecard) use ($fixtureDetailDTO) {
...
//        Log::debug('bowling: ' . ($bowling == null? '[]': json_encode($bowling->toArray(), true)));
    ...
    private function updateUserContestProgress()
    {
        $scs = $this->scorecards;
        $this->usercontests = $this->usercontests->transform(function ($usercontest) use ($scs) {
            $playerIds = array_map(fn($player): int => $player['id'], $usercontest->team->team_members);
            $key_members = $usercontest->team->key_members;
            $playerScorecards = $scs->filter(function ($scorecard) use ($playerIds) {
